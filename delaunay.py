...
EPSILON = math.pow(2,-52) #used for checking the duplicate values
EDGE_STACK =[None] * 512 #storing the edges of the stack
...
def draw_triangle(vertices):
    for i in range(2):
        canvas.create_line(vertices[i][0], vertices[i][1], vertices[i + 1][0], vertices[i + 1][1], fill='black')
    canvas.create_line(vertices[2][0], vertices[2][1], vertices[0][0], vertices[0][1], fill='black')

def calculate_triangulation():
    delau = Delaunator(canvas_points)
    triangles = delau.triangles
    convex_hull = delau.hull  # Get the convex hull points

    for i in range(0, len(triangles), 3):
        draw_triangle([canvas_points[triangles[i]], canvas_points[triangles[i + 1]], canvas_points[triangles[i + 2]]])

    # Calculate guard positions and mark them
    guard_positions = calculate_guard_positions(convex_hull)
    mark_guard_positions(guard_positions)
    
    graph = create_graph(delau.triangles)

    # Check if the graph is 3-colorable
    coloring = is_colorable(graph, 3)

    if coloring is not None:
        # Apply coloring to canvas
        apply_coloring_to_canvas(coloring)
    else:
        print("The graph is not 3-colorable")

def apply_coloring_to_canvas(coloring):
    colors = ['red', 'green', 'blue']
    for node, color in coloring.items():
        point = canvas_points[node]
        canvas.create_oval(point[0] - 3, point[1] - 3, point[0] + 3, point[1] + 3, fill=colors[color])

    
def calculate_guard_positions(hull_points):
    # Choose every third vertex as a guard position
    return [hull_points[i] for i in range(0, len(hull_points), 3)]

def mark_guard_positions(guard_positions):
    for pos in guard_positions:
        point = canvas_points[pos]
        canvas.create_oval(point[0] - 4, point[1] - 4, point[0] + 4, point[1] + 4, fill='green')


def apply_art_gallery_theorem(hull_points):
    number_of_vertices = len(hull_points)
    number_of_guards = math.floor(number_of_vertices / 3)
    print(f"Minimum number of guards needed: {number_of_guards}")
    # Note: This just calculates the number of guards.
    # Determining their exact positions is a more complex problem.

def add_point(event):
    canvas.create_oval(event.x - 2, event.y - 2, event.x + 2, event.y + 2, fill='red')
    canvas_points.append((event.x, event.y))

def clear_canvas():
    canvas.delete("all")
    canvas_points.clear()

def create_graph(triangles):
    graph = {}
    for i in range(0, len(triangles), 3):
        for j in range(3):
            if triangles[i+j] not in graph:
                graph[triangles[i+j]] = set()
            graph[triangles[i+j]].update({triangles[i+(j+1)%3], triangles[i+(j+2)%3]})
    return graph

def is_colorable(graph, max_colors):
...
calculate_button = tk.Button(root, text='Calculate Delaunay Triangulation', command=calculate_triangulation)
